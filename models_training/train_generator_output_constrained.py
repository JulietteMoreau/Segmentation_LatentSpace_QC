...
def calculate_object_size(batch):
    tailles = []
    for b in range(batch.shape[0]):
        tailles.append(batch[b,0,...].sum())
    return torch.Tensor(tailles)
...
##############################################################################################################evaluate
def evaluate(net, dataloader, device, criterion, regression_loss):
    net.eval()
    ...
    for batch in tqdm(dataloader, total=num_val_batches, desc='Validation round', unit='batch', leave=False):
        ...
        area_true = calculate_object_size(mask_true)
        area_true = area_true.cuda()
        ...
        with torch.no_grad():
            ...
            area_pred = calculate_object_size(mask_pred)
            area_pred = area_pred.cuda()
            
            loss = (criterion(mask_pred, mask_true.long().squeeze(1)) + dice_loss(F.softmax(mask_pred, dim=1).float(), F.one_hot(mask_true.long().squeeze(1), net.n_classes).permute(0, 3, 1, 2).float(), multiclass=True)) + regression_loss(area_pred, area_true)
...
def train_net(train_loader, train_ds, val_loader, outdir, patience=5, num_epoch=500,
    ...
    for epoch in range(num_epoch):
        ...
        for i, batch in enumerate(train_loader):
            ...
            area_label = calculate_object_size(real_Mask)
            area_label = area_label.cuda()


            fake_Mask = net(real_CT.float())
            
            fake_area = calculate_object_size(fake_Mask)
            fake_area = fake_area.cuda()
            
            loss = criterion(fake_Mask, real_Mask.long().squeeze(1)) + dice_loss(F.softmax(fake_Mask, dim=1).float(), F.one_hot(real_Mask.long().squeeze(1), net.n_classes).permute(0, 3, 1, 2).float(), multiclass=True) + regression_loss(fake_area, area_label)
        ...
        val_loss, val_score = evaluate(net, val_loader, device, criterion, regression_loss)
        optimizer.zero_grad(set_to_none=True)
